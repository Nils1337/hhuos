...
bool Keyboard::first = true;

void Keyboard::trigger () { 

    // hack to ignore first keyboard interrupt
    // which is triggered in qenu without a key being hit
    if (first) {
        first = false;
        return;
    }
    ...
    Key key = key_hit();
    if (key.valid()) {
        if (key.alt() && key.ctrl() && key.del()) {
            kout << "Ctrl-Alt-Del pressed" << endl;
        } else {
            lastKey = key.ascii();
            kout << key.ascii();
            kout.flush();
